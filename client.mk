...
CONFIG_GUESS_SCRIPT := $(wildcard $(MOZILLA_SRCDIR)/build/autoconf/config.guess)
...
CONFIGURES += $(MOZILLA_SRCDIR)/configure
CONFIGURES += $(MOZILLA_SRCDIR)/js/src/configure
...
ifdef MOZ_BUILD_PROJECTS
MOZ_BUILD_DATE = $(shell $(PYTHON) $(MOZILLA_SRCDIR)/build/variables.py buildid_header | awk '{print $$3}')
...
EXTRA_CONFIG_DEPS := \
	...
	$(MOZILLA_SRCDIR)/aclocal.m4 \
	$(MOZILLA_SRCDIR)/old-configure.in \
	$(wildcard $(MOZILLA_SRCDIR)/build/autoconf/*.m4) \
	$(MOZILLA_SRCDIR)/js/src/aclocal.m4 \
	$(MOZILLA_SRCDIR)/js/src/old-configure.in \
	$(NULL)
...
CONFIG_STATUS_DEPS := \
	$(wildcard $(CONFIGURES)) \
	$(wildcard $(MOZILLA_SRCDIR)/nsprpub/configure) \
	$(wildcard $(MOZILLA_SRCDIR)/config/milestone.txt) \
	$(wildcard $(MOZILLA_SRCDIR)/ldap/sdks/c-sdk/configure) \
